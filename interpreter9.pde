...
var s="";
void get_string(){
  s=document.getElementById("program").value;
}
...
void draw(){
  if(!s.equals("")){
    ...
    document.getElementById("left").innerHTML =
     s + "<br>" + document.getElementById("left").innerHTML;
...
class Print extends ASTList{
  ...
  Object eval(int k, Environment e){
    var right_area = document.getElementById("right");
    var tmp = right_area.innerHTML;
    right_area.innerHTML="";

    Arguments arg = (Arguments)(children.get(0));
    for(int i=0; i<arg.children.size(); i++){
      right_area.innerHTML += arg.children.get(i).eval(k+1, e) + ", ";
    }
    right_area.innerHTML += "<br>" + tmp;
    return null;
  }
}
...
class Environment{
  ...
  void printall(){
    var denv = document.getElementById("env");
    denv.innerHTML="";
    for(String key: hm.keySet()){
      denv.innerHTML += key+": "+hm.get(key)+"<br>";
    }
  }
}
...
public class AssignmentException extends Error {
